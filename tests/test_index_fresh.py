...
def test_index_is_fresh():
    ...
    assert PARTS_MD.read_text(encoding="utf-8") == render_markdown(index), "PARTS.md is stale — run scripts/reindex.py"
    assert json.loads(PARTS_JSON.read_text(encoding="utf-8")) == json.loads(json.dumps(index, default=str)), "parts.json is stale — run scripts/reindex.py"


def test_index_has_locations_and_reverse_deps():
    idx = json.loads(PARTS_JSON.read_text(encoding="utf-8"))
